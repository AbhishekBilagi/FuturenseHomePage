...
function HomeComponent() {
  ...
  return (
    <div className="body">
      ...
      <Modal size="lg" isOpen={modal_apply} toggle={() => setModal_apply(!modal_apply)}>
        <button className="button-close" onClick={() => setModal_apply(!modal_apply)}>
          x
        </button>
        <Apply_form />
      </Modal>
      ...
      {isVisible && 
      <button className="btn btn-apply" onClick={() => setModal_apply(true)}>Apply</button>
      }
      <Navbar />
      <Video />
      <Counter />
